...
export default class AxisPad extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            handler: props.handlerSize ? props.handlerSize : 60,
    ...
    limitter(input) {
        ...
        // return stepped(limited(minimised)) * width / 2; //default value

        // width / 4 is the custom warpper to limitted the small circle in big circle
        return stepped(limited(minimised)) * width / 4;
    }

    sendValue(x, y) {
        const { width } = this.state;
        // this.props.onValue && this.props.onValue({ x: x / width * 2, y: y / width * 2 }); //default value

        // width * 4 is the custom warpper to limitted the small circle in big circle
        this.props.onValue && this.props.onValue({ x: x / width * 4, y: y / width * 4 });
    }
    ...
    onTouchMove(evt) {
        ...
        if (touchItem) {
            ...
            // console.log('*************xxxxx', this.state.px, '**********yyyyyy', py)
        }
    }
